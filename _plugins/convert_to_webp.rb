	...
	Jekyll::Hooks.register :site, :post_write do |site|
		...
		Dir.glob(File.join(site.source, UPLOADS_SOURCE, '*.{gif,jpeg,jpg,png}')).each do |filename|
			...
			if File.extname(filename) == '.gif'
				...
				result = `magick #{Shellwords.escape(filename)} -resize '2000x2000>' -quality 90 -define webp:lossless=false #{Shellwords.escape(webp_file)} 2>&1`

				if $?.success? && File.exist?(webp_file)
					Jekyll.logger.info 'Converted', "#{File.basename(webp_file)}"
				else
					Jekyll.logger.error 'Error', "Failed to convert #{File.basename(filename)}!"
				end
			end
		end
	end
